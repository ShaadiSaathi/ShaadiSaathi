...
function isMoreSectionActive(pathname: string) {
  return (
    ...
    pathname.startsWith("/vendor/upgrade") ||
    pathname.startsWith("/vendor/subscription")
  )
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-3" aria-label="More vendor sections">
          <Link
            href="/vendor/profile"
            onClick={onClose}
            ...
            aria-current={isActive(pathname, "/vendor/profile") ? "page" : undefined}
          >
            <span className={isActive(pathname, "/vendor/profile") ? "text-gold" : "text-gold-dark/70"}>
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.577-1.22a2.25 2.25 0 00-2.134 0z" />
              </svg>
            </span>
          ...
          <div className="mt-3 flex items-center gap-3 rounded-xl bg-white px-3 py-3">
            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-maroon to-maroon-dark text-xs font-semibold text-gold">
              LF
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-maroon-dark">{CURRENT_VENDOR.name}</p>
